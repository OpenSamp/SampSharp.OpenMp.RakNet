...
public class DriverSync(BitStream bs) : ISync
{
    ...
    public void ReadOutcoming()
    {
        ...
        VehicleHealth = BS.ReadUInt16();

        var healthArmour = Convert.ToByte(BS.ReadUInt8());
        ...
        var sirenState = BS.ReadCompressedBool();
        if (sirenState)
            SirenState = 1;

        var landingGear = BS.ReadCompressedBool();
        if (landingGear)
            LandingGearState = 1;

        // HYDRA THRUST ANGLE AND TRAILER Id
        BS.ReadCompressedBool();
          BS.ReadCompressedBool();
         BS.ReadUInt32();
        var train = BS.ReadCompressedBool();

        if (train) TrainSpeed = BS.ReadUInt8();
    }
    ...
    public void WriteOutcoming()
    {
        ...
        var healthArmour = HealthArmour.SetInByte(PlayerHealth, PlayerArmour);
        BS.WriteUInt8(healthArmour);
        BS.WriteUInt8(WeaponId);

        BS.WriteBool(SirenState == 1);

        BS.WriteBool(LandingGearState == 1);

        // HYDRA THRUST ANGLE AND TRAILER Id
        BS.WriteBool(false);
        BS.WriteBool(false);

        const int trailerIdOrThrustAngle = 0;
        BS.WriteUInt32(trailerIdOrThrustAngle);
