...
namespace SampSharp.RakNet.Entities.Pawn.Syncs;

public class OnFootSync(BitStream bs) : ISync
{
    public BitStream BS { get; set; } = bs;
    ...
    public void ReadIncoming()
    {
        ...
        Quaternion = new Vector4((float)result["quaternion_X"], (float)result["quaternion_Y"],
            (float)result["quaternion_Z"], (float)result["quaternion_W"]); // order is different from one in a bitstream
        ...
        SurfingOffsets = new Vector3((float)result["surfingOffsets_0"], (float)result["surfingOffsets_1"],
            (float)result["surfingOffsets_2"]);
    ...
    public void ReadOutcoming()
    {
        ...
        //LEFT/RIGHT KEYS
        var hasLR = BS.ReadBool();
        if (hasLR) LRKey = BS.ReadUInt16();

        // UP/DOWN KEYS
        var hasUD = BS.ReadBool();
        ...
        var healthArmour = Convert.ToByte((int)result["healthArmourByte"]);
        HealthArmour.GetFromByte(healthArmour, out var health, out var armour);
        Health = health;
        Armour = armour;
        WeaponId = (int)result["weaponId"];
        SpecialAction = (int)result["specialAction"];
        ...
        var hasSurfInfo = BS.ReadBool();
        if (hasSurfInfo)
        {
            SurfingVehicleId = BS.ReadUInt16();
            var offsetsX = BS.ReadFloat();
            var offsetsY = BS.ReadFloat();
            var offsetsZ = BS.ReadFloat();
        ...
        var hasAnimation = BS.ReadBool();
        if (hasAnimation) AnimationId = BS.ReadInt32();
    }
    ...
    public void WriteOutcoming()
    {
        ...
        var healthArmourByte = HealthArmour.SetInByte(Health, Armour);
        BS.WriteValue(
